...
@HiltViewModel
class DetailPopularMovieVM @Inject constructor(
    private val moviesUseCase: MoviesUseCase,
    private val coroutineThread: CoroutineThread,
    private val popularFavouriteMovieReqMapper: BaseMapper<PopularMovieResUI, FavouriteMovieReq>
) : BaseViewModel() {
    ...
    fun insertFavouriteMovie(popularMovieResUI: PopularMovieResUI) {
        viewModelScope.launch(coroutineThread.background()) {
            val favouriteMovieReq = popularFavouriteMovieReqMapper.map(popularMovieResUI)
